...
class JoystickNode:
	...
	def setNavGoal(self, forward, right):
		...
		x = forward * dist
		y = right * dist
        if self.targetPoseArrayCursor >= 8:
			x += self.targetPoseArray.poses[self.targetPoseArrayCursor - 8][0].position.x
			y += self.targetPoseArray.poses[self.targetPoseArrayCursor - 8][0].position.y
        angles = [0, pi/4.0, pi/2.0, 3*pi/4.0, -pi, -3*pi/4.0, -pi/2.0, -pi/4.0]
		if forward == 1 and right == 0:
            angIndex = 0
        elif forward == 1 and right == 1:
            angIndex = 1
        elif forward == 0 and right == 1:
			angIndex = 2
        elif forward == -1 and right == 1:
            angIndex = 3
        elif forward == -1 and right == 0:
			angIndex = 4
        elif forward == -1 and right == -1:
            angIndex = 5
        elif forward == 0 and right == -1:
			...
			angIndex = 7
        for i in range (angIndex, angIndex + 9):
            g = Pose()
            g.position.x = x
            g.position.y = y
            g.position.z = 0
            q = tf.transformations.quaternion_from_euler(0, 0, angles[i % 8])
            g.orientation.x = q[0]
            g.orientation.y = q[1]
            g.orientation.z = q[2]
            g.orientation.w = q[3]	
            if len(self.targetPoseArray.poses) <= self.targetPoseArrayCursor + i - angIndex :
                self.targetPoseArray.poses.append(g)
            else:
                self.targetPoseArray.poses[self.targetPoseArrayCursor + i - angIndex] = g
